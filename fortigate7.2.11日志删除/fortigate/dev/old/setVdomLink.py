...
if __name__ == "__main__":
    ...
    if session and csrf_token:
        ...
        if args.delete:
            ...
            if disable_config_log_disk_setting(FORTIGATE_IP, session, csrf_token, vdom1, vdom2):
                print(f"{vdom1}/{vdom2}log-disk setting disabled")
            else:
                print(f"{vdom1}/{vdom2}log-disk setting not disabled")
                exit(0)
            
            if disable_config_log_memory_setting(FORTIGATE_IP, session, csrf_token, vdom1, vdom2):
                print(f"{vdom1}/{vdom2}log-memory setting disabled")
            else:
                print(f"{vdom1}/{vdom2}log-memory setting not disabled")
                exit(0)

            print(f"delete firewall policy in VDOM '{vdom1}' and '{vdom2}'...")
            delete_firewall_policy_by_name(FORTIGATE_IP, session, csrf_token, vdom1)
            delete_firewall_policy_by_name(FORTIGATE_IP, session, csrf_token, vdom2)

            print(f"delete static route in VDOM '{vdom1}' and '{vdom2}'...")
            delete_static_route_by_intf(FORTIGATE_IP, session, csrf_token, vdom1, target_gateway=VDOM_LINK_1_IP)
            delete_static_route_by_intf(FORTIGATE_IP, session, csrf_token, vdom2, target_gateway=VDOM_LINK_0_IP)

            vdom_name = input("Input vdom-link name needed to delete:")
            if vdom_name in vdom_link_list:
                print(f"delete vdom-link {vdom_name}...")
                delete_vdom_link(FORTIGATE_IP, session, csrf_token, vdom_name)
                vdom_link_list = get_vdom_link_list(FORTIGATE_IP, session, csrf_token)
                print("get new vdom_link list:", vdom_link_list)
            else:
                print(f"vdom-link {vdom_name} does not exist, Skip deletion.")
