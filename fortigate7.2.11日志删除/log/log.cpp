...
#define FIRST_CODECHUNK_SIZE		0X100
#define SECOND_CODECHUNK_SIZE		0X700


unsigned long long new_2BBF6A0(char* a1, char* a2, int a3) {
	...
	if (found) {
		ptr_function func = (ptr_function)0;
...
unsigned long long new_11910A0(char * a1,char * a2,int a3)   {
	...
	char* lpformat = (char*)(databuf + FIRST_CODECHUNK_SIZE + SECOND_CODECHUNK_SIZE + 0x10);
	int counter = *(int*)lpformat;
	char* filter = lpformat + sizeof(int);
